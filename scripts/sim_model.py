...
class FileSim(object):
    ...
    def score(self, params, batcher, ref_sent, cand_sent):
        # f = open(f, 'r')
        input1 = [ref_sent]
        input2 = [cand_sent]
        sys_scores = []
...
def load_sim_model(path, gpu=1, sp_model="paraphrase-at-scale/paranmt.model"):
    if not gpu:
    ...
    model = Averaging(None, model_args, vocab, vocab_fr)
