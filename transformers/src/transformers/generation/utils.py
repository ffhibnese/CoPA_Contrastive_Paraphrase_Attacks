...
class GenerationMixin:
    ...
    def _validate_model_kwargs(self, model_kwargs: Dict[str, Any]):
        ...
        model_args |= set(['model_inputs_machine', 'lamda', 'alpha'])
    ...
    def generate(
        ...
        **kwargs,
        ...
        flag = True
        try:
            # Add: get machine prompt
            inputs_aux = model_kwargs.pop('model_inputs_machine')
            # Add: get lamda and alpha
            lamda_param = model_kwargs.pop('lamda')
            alpha_param = model_kwargs.pop('alpha')
        except:
            flag = False
            # logger.warning('model_inputs_machine or lamda not found in model_kwargs')
        ...
        if flag:
            inputs_tensor_aux = inputs_aux['input_ids']
            attention_mask_aux = inputs_aux['attention_mask']
        ...
        if self.config.is_encoder_decoder:
            ...
            # Add
            if flag:
                input_ids_aux = inputs_tensor_aux 
        if generation_config.token_healing:
            input_ids = self.heal_tokens(input_ids, tokenizer)
            # Add
            if flag:
                input_ids_aux = self.heal_tokens(input_ids_aux, tokenizer)
        ...
        # Add
        if flag:
            prepared_logits_processor_aux = self._get_logits_processor(
                generation_config=generation_config,
                input_ids_seq_length=input_ids_length,
                encoder_input_ids=inputs_tensor_aux,
                prefix_allowed_tokens_fn=prefix_allowed_tokens_fn,
                logits_processor=logits_processor,
                device=inputs_tensor_aux.device,
                model_kwargs=model_kwargs,
                negative_prompt_ids=negative_prompt_ids,
                negative_prompt_attention_mask=negative_prompt_attention_mask,
            )
        ...
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            ...
            # Add
            if flag:
                input_ids_aux, model_kwargs = self._expand_inputs_for_generation(
                    input_ids=input_ids_aux,
                    expand_size=generation_config.num_return_sequences,
                    is_encoder_decoder=self.config.is_encoder_decoder,
                    **model_kwargs,
                )
                model_kwargs['input_ids_aux'] = input_ids_aux
                model_kwargs['attention_mask_aux'] = attention_mask_aux
                model_kwargs['lamda'] = lamda_param
                model_kwargs['alpha'] = alpha_param
            model_kwargs['flag'] = flag
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        has_eos_stopping_criteria = any(hasattr(criteria, "eos_token_id") for criteria in stopping_criteria)
        do_sample = generation_config.do_sample
        flag = model_kwargs.pop('flag', False)
        # Add
        if flag:
            input_ids_aux = model_kwargs.pop('input_ids_aux')
            attention_mask_aux = model_kwargs.pop('attention_mask_aux')
            lamda_param = model_kwargs.pop('lamda')
            alpha_param = model_kwargs.pop('alpha')
            model_kwargs_aux = copy.deepcopy(model_kwargs)
            model_kwargs_aux['attention_mask'] = attention_mask_aux
        ...
        # Add
        if flag:
            model_kwargs_aux = self._get_initial_cache_position(input_ids_aux, model_kwargs_aux)
            batch_size_aux, cur_len_aux = input_ids_aux.shape
            this_peer_finished_aux = False 
            unfinished_sequences_aux = torch.ones(batch_size_aux, dtype=torch.long, device=input_ids_aux.device)
            finished_aux = False
        ...
        while self._has_unfinished_sequences(
            ...
            # Add
            if flag and not finished_aux:
                finished_aux = self._has_unfinished_sequences(this_peer_finished_aux, synced_gpus, 
                                                              device=input_ids_aux.device, cur_len=cur_len_aux, max_length=max_length)
                model_inputs_aux = self.prepare_inputs_for_generation(input_ids_aux, **model_kwargs_aux)
                model_inputs_aux.update({"output_attentions": output_attentions} if output_attentions else {})
                model_inputs_aux.update({"output_hidden_states": output_hidden_states} if output_hidden_states else {})
                outputs_aux = self(**model_inputs_aux, return_dict=True)
                model_kwargs_aux = self._update_model_kwargs_for_generation(
                    outputs_aux,
                    model_kwargs_aux,
                    is_encoder_decoder=self.config.is_encoder_decoder,
                )
                next_token_logits_aux = outputs_aux.logits.clone()[:, -1, :].float()
                next_token_logits_aux = next_token_logits_aux.to(input_ids_aux.device)
                cutoff = torch.log(torch.tensor(alpha_param)) + next_token_logits.max(dim=-1, keepdim=True).values
                diffs = (1 + lamda_param) * next_token_logits - lamda_param * next_token_logits_aux
                if lamda_param > 0:
                    next_token_logits = diffs.masked_fill(next_token_logits < cutoff, -float("inf"))
                else:
                    next_token_logits = diffs
            # pre-process distribution
            next_token_scores = logits_processor(input_ids, next_token_logits)
            ...
            # Add
            if flag and not finished_aux:
                input_ids_aux = torch.cat([input_ids_aux, next_tokens[:, None]], dim=-1)
            if streamer is not None:
                streamer.put(next_tokens.cpu())
            ...
            # Add
            if flag and not finished_aux:
                unfinished_sequences_aux = unfinished_sequences_aux & ~stopping_criteria(input_ids_aux, scores)
                this_peer_finished_aux = unfinished_sequences_aux,max() == 0
                cur_len_aux += 1
